Like that as an agent listed in the integration agent folder and integrations Tool-tui/crates/agent folder that DX can spawn an agent that can run in the background, both locally and VPS And it can run there 24/7. Latest VIRAL projects like zeroClaw, openClaw are doing it so we have to support that out of the box. Not only that, our DX can also handle real computer using the best latest crates. And we will not only show co-editor when someone opens our app but also we will show a Wispr Flow-like bottom center sound text. You can learn from the integrations 11 labs UI for the GPU implementation of the sound-to-text. Whenever a special hotkey will be pressed it will show a bottom center Wispr Flow-like rounded feel to get the audio. In the integrations www dashboard token folder, we day/app/website folder, you can see the Ai agent face in the landing page top hero section. We have to recreate it in Rust GPU. By default on screen we will show that in the air face in the bottom center, like what was for Flow does, and when the AI is clicked it will open a small instance of our Ai panel flowing into the users operating system. We can run a tiny LLM model for the sound-to-text, unlimited free, and also can run a small Ollama model in the user's operating system to run 24/7 and control the user's operating system correctly.
