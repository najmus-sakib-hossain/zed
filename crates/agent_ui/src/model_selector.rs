...
1. Enum renamed from `ModelPickerEntry` to `AcpModelPickerEntry`
2. Separator variant changed from:
   - Incoming: Separator(SharedString)
   - Local:    Separator { title: SharedString, model_count: usize }

3. Struct renamed from `ModelPickerDelegate` to `AcpModelPickerDelegate`

Key differences in the Separator variant usage:

LOCAL VERSION (with model_count):
```rust
...
// Creating separators with model count:
...
If you need to restore the model_count feature:
1. Change enum back to AcpModelPickerEntry with Separator { title, model_count }
2. Update all pattern matches to use the struct variant
3. Pass model_count when creating separators
4. Update ModelSelectorHeader to display model_count
