...
fn whatsapp_signature_rejects_missing_sha256_prefix() {
    ...
    assert!(!dx::gateway::verify_whatsapp_signature(
        secret, body, bad_sig
    ));
}
...
fn whatsapp_signature_rejects_empty_signature() {
    ...
    assert!(!dx::gateway::verify_whatsapp_signature(secret, body, ""));
}
...
fn whatsapp_signature_different_secrets_produce_different_sigs() {
    ...
    assert!(dx::gateway::verify_whatsapp_signature(secret1, body, &sig1));
    assert!(!dx::gateway::verify_whatsapp_signature(
        secret2, body, &sig1
    ));
    assert!(dx::gateway::verify_whatsapp_signature(secret2, body, &sig2));
    assert!(!dx::gateway::verify_whatsapp_signature(
        secret1, body, &sig2
    ));
}
