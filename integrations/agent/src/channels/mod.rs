...
//! DX to external platforms. Each channel implements the [`Channel`] trait
...
const SYSTEMD_STATUS_ARGS: [&str; 3] = ["--user", "is-active", "dx.service"];
const SYSTEMD_RESTART_ARGS: [&str; 3] = ["--user", "restart", "dx.service"];
const OPENRC_STATUS_ARGS: [&str; 2] = ["dx", "status"];
const OPENRC_RESTART_ARGS: [&str; 2] = ["dx", "restart"];
    ...
    match channel_name {
        ...
        "whatsapp" => Some(
            "When responding on WhatsApp:\n\
             ...
             - The path MUST be an absolute filesystem path to a local file (e.g. [IMAGE:/home/nicolas/.dx/workspace/images/chart.png]).\n\
             - Keep normal text outside markers and never wrap markers in code fences.\n\
...
async fn load_runtime_defaults_from_config_file(
    ...
    if let Some(dx_dir) = path.parent() {
        let store = crate::security::SecretStore::new(dx_dir, parsed.secrets.encrypt);
...
fn build_models_help_response(current: &ChannelRouteSelection, workspace_dir: &Path) -> String {
    ...
    if cached_models.is_empty() {
        let _ = writeln!(
            response,
            "\nNo cached model list found for `{}`. Ask the operator to run `dx models refresh --provider {}`.",
...
pub fn build_system_prompt_with_mode(
    ...
    if prompt.is_empty() {
        "You are DX, a fast and efficient AI assistant built in Rust. Be helpful, concise, and direct."
            .to_string()
    } else {
        prompt
    }
...
async fn bind_telegram_identity(config: &Config, identity: &str) -> Result<()> {
    ...
    let Some(telegram) = updated.channels_config.telegram.as_mut() else {
        anyhow::bail!(
            "Telegram channel is not configured. Run `dx onboard --channels-only` first"
        );
    };
    ...
    match maybe_restart_managed_daemon_service() {
        ...
        Ok(false) => {
            println!(
                "ℹ️ No managed daemon service detected. If `dx daemon`/`channel start` is already running, restart it to load the updated allowlist."
            );
        }
        Err(e) => {
            eprintln!(
                "⚠️ Allowlist saved, but failed to reload daemon service automatically: {e}\n\
                 Restart service manually with `dx service stop && dx service start`."
            );
        }
    }
    Ok(())
...
fn maybe_restart_managed_daemon_service() -> Result<bool> {
    if cfg!(target_os = "macos") {
        ...
        if !listed.contains("com.dx.daemon") {
            return Ok(false);
        }

        let _ = Command::new("launchctl")
            .args(["stop", "com.dx.daemon"])
            .output();
        let start_output = Command::new("launchctl")
            .args(["start", "com.dx.daemon"])
            .output()
    ...
    if cfg!(target_os = "linux") {
        ...
        let openrc_init_script = PathBuf::from("/etc/init.d/dx");
...
pub(crate) async fn handle_command(command: crate::ChannelCommands, config: &Config) -> Result<()> {
    match command {
        ...
        crate::ChannelCommands::List => {
            ...
            println!("\nTo start channels: dx channel start");
            println!("To check health:    dx channel doctor");
            println!("To configure:      dx onboard");
            Ok(())
        }
        crate::ChannelCommands::Add {
            ...
            anyhow::bail!(
                "Channel type '{channel_type}' — use `dx onboard` to configure channels"
            );
        }
        crate::ChannelCommands::Remove { name } => {
            anyhow::bail!("Remove channel '{name}' — edit ~/.dx/config.toml directly");
        }
...
pub async fn doctor_channels(config: Config) -> Result<()> {
    ...
    if channels.is_empty() && init_failures.is_empty() {
        println!("No real-time channels configured. Run `dx onboard` first.");
        return Ok(());
    }

    println!("🩺 DX Channel Doctor");
    println!();
    ...
    if config.channels_config.webhook.is_some() {
        println!("  ℹ️  Webhook   check via `dx gateway` then GET /health");
    }
...
pub async fn start_channels(config: Config) -> Result<()> {
    ...
    if configured_channels.is_empty() && init_failures.is_empty() {
        println!("No channels configured. Run `dx onboard` to set up channels.");
        return Ok(());
    }
...
BTC is currently around $65,000 based on latest tool output."#
    ...
    async fn process_channel_message_handles_approve_command_without_llm_call() {
        ...
        let runtime_ctx = Arc::new(ChannelRuntimeContext {
            ...
            reliability: Arc::new(crate::config::ReliabilityConfig::default()),
            provider_runtime_options: providers::ProviderRuntimeOptions {
                dx_dir: Some(temp.path().to_path_buf()),
    ...
    fn prompt_injects_workspace_files() {
        ...
        assert!(prompt.contains("Name: DX"), "missing IDENTITY content");
    ...
    fn channel_log_truncation_is_utf8_safe_for_multibyte_text() {
        let msg = "Hello from DX 🌍. Current status is healthy, and café-style UTF-8 text stays safe in logs.";
...
BTC is currently around $65,000 based on latest tool output."#;
    ...
    fn maybe_restart_daemon_systemd_args_regression() {
        assert_eq!(
            SYSTEMD_STATUS_ARGS,
            ["--user", "is-active", "dx.service"]
        );
        assert_eq!(
            SYSTEMD_RESTART_ARGS,
            ["--user", "restart", "dx.service"]
        );
    }

    #[test]
    fn maybe_restart_daemon_openrc_args_regression() {
        assert_eq!(OPENRC_STATUS_ARGS, ["dx", "status"]);
        assert_eq!(OPENRC_RESTART_ARGS, ["dx", "restart"]);
    }
