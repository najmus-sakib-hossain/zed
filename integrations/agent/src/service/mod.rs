...
const SERVICE_LABEL: &str = "com.dx.daemon";
const WINDOWS_TASK_NAME: &str = "DX Daemon";
...
fn start_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            ...
            run_checked(Command::new("systemctl").args(["--user", "start", "dx.service"]))?;
        }
        InitSystem::Openrc => {
            run_checked(Command::new("rc-service").args(["dx", "start"]))?;
        }
...
fn stop_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            let _ =
                run_checked(Command::new("systemctl").args(["--user", "stop", "dx.service"]));
        }
        InitSystem::Openrc => {
            let _ = run_checked(Command::new("rc-service").args(["dx", "stop"]));
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    println!("✅ Service stopped");
...
fn restart_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            run_checked(Command::new("systemctl").args(["--user", "daemon-reload"]))?;
            run_checked(Command::new("systemctl").args(["--user", "restart", "dx.service"]))?;
        }
        InitSystem::Openrc => {
            run_checked(Command::new("rc-service").args(["dx", "restart"]))?;
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    println!("✅ Service restarted");
...
fn status_linux(config: &Config, init_system: InitSystem) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            let out = run_capture(Command::new("rc-service").args(["dx", "status"]))
                .unwrap_or_else(|_| "unknown".into());
            println!("Service state: {}", out.trim());
            println!("Unit: /etc/init.d/dx");
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    Ok(())
...
fn uninstall_linux(config: &Config, init_system: InitSystem) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            let init_script = Path::new("/etc/init.d/dx");
            if init_script.exists() {
                if let Err(err) =
                    run_checked(Command::new("rc-update").args(["del", "dx", "default"]))
                {
                    eprintln!(
                        "⚠️  Warning: Could not remove dx from OpenRC default runlevel: {err}"
                    );
                }
                fs::remove_file(init_script)
                    .with_context(|| format!("Failed to remove {}", init_script.display()))?;
            }
            println!("✅ Service uninstalled (/etc/init.d/dx)");
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    Ok(())
...
"#,
    ...
    println!("   Start with: dx service start");
    Ok(())
}
...
fn install_linux_systemd(config: &Config) -> Result<()> {
    ...
    fs::write(&file, unit)?;
    let _ = run_checked(Command::new("systemctl").args(["--user", "daemon-reload"]));
    let _ = run_checked(Command::new("systemctl").args(["--user", "enable", "dx.service"]));
...
/// Check if the dx user exists and has expected properties.
...
fn check_dx_user() -> Result<()> {
    let output = Command::new("getent").args(["passwd", "dx"]).output();
    let is_alpine = Path::new("/etc/alpine-release").exists();

    let (del_cmd, add_cmd) = if is_alpine {
        (
            "deluser dx && delgroup dx",
            "addgroup -S dx && adduser -S -s /sbin/nologin -H -D -G dx dx",
        )
    } else {
        ("userdel dx", "useradd -r -s /sbin/nologin dx")
    };
        ...
        Ok(output) if output.status.success() => {
            ...
            if parts.len() >= 7 {
                ...
                if uid.parse::<u32>().unwrap_or(999) >= 1000 {
                    bail!(
                        "User 'dx' exists but has unexpected UID {} (expected system UID < 1000).\n\
                ...
                if !shell.contains("nologin") && !shell.contains("false") {
                    bail!(
                        "User 'dx' exists but has unexpected shell '{}'.\n\
                ...
                if home != "/var/lib/dx" && home != "/nonexistent" {
                    eprintln!(
                        "⚠️  Warning: dx user has home directory '{}' (expected /var/lib/dx or /nonexistent)",
                        home
                    );
                }
...
fn ensure_dx_user() -> Result<()> {
    let output = Command::new("getent").args(["passwd", "dx"]).output();
    if let Ok(output) = output {
        if output.status.success() {
            return check_dx_user();
        }
    }
    ...
    if is_alpine {
        let group_output = Command::new("getent").args(["group", "dx"]).output();
        ...
        if !group_exists {
            let output = Command::new("addgroup")
                .args(["-S", "dx"])
                .output()
                .context("Failed to create dx group")?;

            if !output.status.success() {
                let stderr = String::from_utf8_lossy(&output.stderr);
                bail!("Failed to create dx group: {}", stderr.trim());
            }
            println!("✅ Created system group: dx");
        }

        let output = Command::new("adduser")
            .args(["-S", "-s", "/sbin/nologin", "-H", "-D", "-G", "dx", "dx"])
            .output()
            .context("Failed to create dx user")?;

        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            bail!("Failed to create dx user: {}", stderr.trim());
        }
    } else {
        let output = Command::new("useradd")
            .args(["-r", "-s", "/sbin/nologin", "dx"])
            .output()
            .context("Failed to create dx user")?;
    ...
    println!("✅ Created system user: dx");
    Ok(())
}

/// Change ownership of a path to dx:dx
#[cfg(unix)]
fn chown_to_zeroclaw(path: &Path) -> Result<()> {
    let output = Command::new("chown")
        .args(["dx:dx", &path.to_string_lossy()])
        .output()
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Failed to change ownership of {} to dx:dx: {}",
            path.display(),
            stderr.trim(),
        );
    }
...
fn chown_recursive_to_zeroclaw(path: &Path) -> Result<()> {
    let output = Command::new("chown")
        .args(["-R", "dx:dx", &path.to_string_lossy()])
        .output()
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Failed to recursively change ownership of {} to dx:dx: {}",
            path.display(),
            stderr.trim(),
        );
    }
    ...
    if let Some(user) = sudo_user {
        if let Ok(output) = Command::new("getent").args(["passwd", &user]).output() {
            if output.status.success() {
                ...
                if fields.len() >= 6 {
                    return Some(PathBuf::from(fields[5]).join(".dx"));
                }
            }
        }
    }
...
fn ensure_openrc_runtime_path_writable(path: &Path) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        bail!(
            "OpenRC runtime user 'dx' cannot write {} ({details}). \
             Re-run `sudo dx service install` and ensure ownership is dx:dx.",
            path.display(),
        );
    }
...
umask 027
output_log="/var/log/dx/access.log"
error_log="/var/log/dx/error.log"
...
fn resolve_openrc_executable() -> Result<PathBuf> {
    let preferred = Path::new("/usr/local/bin/dx");
    if preferred.exists() {
...
fn install_linux_openrc(config: &Config) -> Result<()> {
    if !is_root() {
        bail!(
            "OpenRC service installation requires root privileges.\n\
             Please run with sudo: sudo dx service install"
        );
    }
    ...
    let config_dir = Path::new("/etc/dx");
    let workspace_dir = config_dir.join("workspace");
    let log_dir = Path::new("/var/log/dx");
    ...
    if !workspace_dir.exists() {
        ...
        println!(
            "✅ Created directory: {} (owned by dx:dx)",
    ...
    let init_path = Path::new("/etc/init.d/dx");
    ...
    run_checked(Command::new("rc-update").args(["add", "dx", "default"]))?;
    println!("✅ Installed OpenRC service: /etc/init.d/dx");
    println!("   Config path: /etc/dx/config.toml");
    println!("   Start with: sudo dx service start");
    let _ = config;
    Ok(())
}
...
fn install_windows(config: &Config) -> Result<()> {
    ...
    let wrapper = logs_dir.join("dx-daemon.cmd");
...
mod tests {
    ...
    fn linux_service_file_has_expected_suffix() {
        ...
        assert!(path.ends_with(".config/systemd/user/dx.service"));
    }

    #[test]
    fn windows_task_name_is_constant() {
        assert_eq!(windows_task_name(), "DX Daemon");
    }
    ...
    fn generate_openrc_script_contains_required_directives() {
        ...
        let exe_path = PathBuf::from("/usr/local/bin/dx");
        let script = generate_openrc_script(&exe_path, Path::new("/etc/dx"));

        assert!(script.starts_with("#!/sbin/openrc-run"));
        assert!(script.contains("name=\"dx\""));
        assert!(script.contains("description=\"DX daemon\""));
        assert!(script.contains("command=\"/usr/local/bin/dx\""));
        assert!(script.contains("command_args=\"--config-dir /etc/dx daemon\""));
        assert!(!script.contains("env DX_CONFIG_DIR"));
        assert!(!script.contains("env DX_WORKSPACE"));
        assert!(script.contains("command_background=\"yes\""));
        assert!(script.contains("command_user=\"dx:dx\""));
        assert!(script.contains("pidfile=\"/run/${RC_SVCNAME}.pid\""));
        assert!(script.contains("umask 027"));
        assert!(script.contains("output_log=\"/var/log/dx/access.log\""));
        assert!(script.contains("error_log=\"/var/log/dx/error.log\""));
    ...
    fn openrc_writability_probe_prefers_runuser_when_available() {
        let (program, args) =
            build_openrc_writability_probe_command(Path::new("/etc/dx"), true);
    ...
    fn openrc_writability_probe_falls_back_to_su() {
        let (program, args) =
            build_openrc_writability_probe_command(Path::new("/etc/dx/workspace"), false);
        ...
        assert_eq!(
            ...
            vec![
                ...
                "/bin/sh".to_string(),
                "-c".to_string(),
                "test -w '/etc/dx/workspace'".to_string(),
                "dx".to_string()
            ]
        );
    }
