//! Flash DX Nucleo-F401RE firmware via probe-rs.
//!
...
/// Flash DX Nucleo firmware. Builds from firmware/dx-nucleo.
pub fn flash_nucleo_firmware() -> Result<()> {
    ...
    // CARGO_MANIFEST_DIR = repo root (dx's Cargo.toml)
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let firmware_dir = repo_root.join("firmware").join("dx-nucleo");
    if !firmware_dir.join("Cargo.toml").exists() {
        anyhow::bail!(
            "Nucleo firmware not found at {}. Run from dx repo root.",
            firmware_dir.display()
        );
    }

    println!("Building DX Nucleo firmware...");
    ...
    println!("DX Nucleo firmware flashed successfully.");
