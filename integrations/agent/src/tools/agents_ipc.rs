//! Inter-process communication tools for independent DX agents.
//!
//! Provides 5 LLM-callable tools backed by a shared SQLite database, allowing
//! independent DX processes on the same host to discover each other and
...
/// Shared SQLite handle for IPC tools. Each DX process holds one instance.
