...
impl SyscallAnomalyDetector {
    ...
    pub fn new(
        config: SyscallAnomalyConfig,
        dx_dir: impl AsRef<Path>,
        audit_config: AuditConfig,
    ) -> Self {
        let baseline = normalize_baseline(&config.baseline_syscalls);
        let anomaly_log_path = resolve_log_path(dx_dir.as_ref(), config.log_path.as_str());
        let audit_logger = AuditLogger::new(audit_config, dx_dir.as_ref().to_path_buf()).ok();
