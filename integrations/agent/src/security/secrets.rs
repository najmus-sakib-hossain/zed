...
// in `~/.dx/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`~/.dx/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(dx_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: dx_dir.join(".secret_key"),
            enabled,
        }
    }
