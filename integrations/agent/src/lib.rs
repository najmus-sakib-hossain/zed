...
Examples:
  ...
  dx channel add discord '{\"bot_token\":\"...\",\"name\":\"my-discord\"}'")]
    Add {
...
Examples:
  ...
  dx channel bind-telegram 123456789")]
    BindTelegram {
...
pub enum MigrateCommands {
    /// Import memory from an `OpenClaw` workspace into this `DX` workspace
    Openclaw {
...
Examples:
  ...
  dx cron add-at 2025-12-31T23:59:00Z 'Happy New Year!'")]
    AddAt {
...
Examples:
  dx cron add-every 60000 'Ping heartbeat'     # every minute
  dx cron add-every 3600000 'Hourly report'    # every hour")]
    AddEvery {
...
Examples:
  ...
  dx cron once 2h 'Follow up on deployment'
  dx cron once 1d 'Daily check'")]
    Once {
        /// Delay duration
        delay: String,
        /// Command to run
...
Examples:
  dx cron update <task-id> --expression '0 8 * * *'
  dx cron update <task-id> --tz Europe/London --name 'Morning check'
  dx cron update <task-id> --command 'Updated message'")]
    Update {
        /// Task ID
        id: String,
        /// New cron expression
...
Examples:
  ...
  dx peripheral add esp32 /dev/ttyUSB0")]
    Add {
    ...
    /// Flash DX firmware to Arduino (creates .ino, installs arduino-cli if needed, uploads)
    #[command(long_about = "\
Flash DX firmware to an Arduino board.
...
Examples:
  ...
  dx peripheral flash -p COM3")]
    ...
    /// Flash DX firmware to Nucleo-F401RE (builds + probe-rs run)
    FlashNucleo,
}
