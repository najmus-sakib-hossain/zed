...
//!
//! Provides two endpoints for callers migrating from OpenClaw to DX:
//!
//! 1. **`POST /api/chat`** (recommended) — DX-native endpoint that invokes the
...
//! DX's existing `/v1/chat/completions` (in `openai_compat.rs`) uses the
...
pub struct ApiChatBody {
    ...
    /// Use this to inject recent conversation history that DX's
...
/// `POST /v1/chat/completions` — OpenAI-compatible shim over DX's agent loop.
///
...
/// the same tools + memory experience as native DX channels.
