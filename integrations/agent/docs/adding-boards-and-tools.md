# Adding Boards and Tools — DX Hardware Guide

This guide explains how to add new hardware boards and custom tools to DX.

## Quick Start: Add a Board via CLI

```bash
# Add a board (updates ~/.dx/config.toml)
dx peripheral add nucleo-f401re /dev/ttyACM0
dx peripheral add arduino-uno /dev/cu.usbmodem12345
dx peripheral add rpi-gpio native   # for Raspberry Pi GPIO (Linux)
...
With the `rag-pdf` feature, DX can index PDF files:
...
2. **Add to config** — `dx peripheral add my-board /dev/ttyUSB0`
...
| Command | Description |
|---------|-------------|
| `dx peripheral list` | List configured boards |
| `dx peripheral add <board> <path>` | Add board (writes config) |
| `dx peripheral flash` | Flash Arduino firmware |
| `dx peripheral flash-nucleo` | Flash Nucleo firmware |
| `dx hardware discover` | List USB devices |
| `dx hardware info` | Chip info via probe-rs |
