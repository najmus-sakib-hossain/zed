...
It is intentionally architecture-focused and keeps DX core boundaries stable.

## Recommendation

Use the plugin/adapter route first.

- Keep robotics transport in an integration crate or module that bridges ROS topics/services/actions to DX tools/channels/runtime adapters.
- Keep high-frequency control loops in ROS-native execution contexts.
- Use DX for planning, orchestration, policy, and guarded action dispatch.
...
- Security posture: approval, policy, and gating remain centralized in existing DX paths.
...
- DX emits intent-level commands and receives summarized state.
...
2. Bridge performs local reduction/windowing and forwards compact summaries to DX.
3. DX decides intent/tool calls under existing policy and approval constraints.
4. Bridge translates approved intents into ROS commands with bounded command-rate limits.
5. Telemetry and fault states flow back into DX for reasoning and auditability.

## Escalation Criteria For Core Integration

Consider deeper DX runtime integration only when all are true:
