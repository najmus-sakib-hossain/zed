...
This guide covers native Nextcloud Talk integration for DX.
...
Add this section in `~/.dx/config.toml`:
...
dx daemon
# or
dx gateway --host 127.0.0.1 --port 3000
```
...
When `webhook_secret` is configured, DX verifies:
...
- DX ignores bot-originated webhook events (`actorType = bots`).
- DX ignores non-message/system events.
...
3. Confirm DX receives and replies in the same room.
