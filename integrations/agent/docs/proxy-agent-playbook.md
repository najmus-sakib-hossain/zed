...
- **Audience:** operators and maintainers running DX in proxied networks.
- **Scope:** `proxy_config` actions, mode selection, verification flow, and troubleshooting.
- **Non-goals:** generic network debugging outside DX runtime behavior.
...
### 1.1 Proxy only DX internal traffic
...
- [Section 4](#4-mode-a--proxy-only-for-dx-internals)
...
|---|---|---|---|
| `dx` | DX internal HTTP clients | No | Normal runtime proxying without process-level side effects |
...
## 4. Mode A — Proxy Only for DX Internals

Use when DX provider/channel/tool HTTP traffic should use proxy, without exporting process-level proxy env vars.
...
- Runtime proxy is active for DX HTTP clients.
