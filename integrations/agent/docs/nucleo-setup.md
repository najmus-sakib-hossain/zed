# DX on Nucleo-F401RE — Step-by-Step Guide

Run DX on your Mac or Linux host. Connect a Nucleo-F401RE via USB. Control GPIO (LED, pins) via Telegram or CLI.

---

## Get Board Info via Telegram (No Firmware Needed)

DX can read chip info from the Nucleo over USB **without flashing any firmware**. Message your Telegram bot:
...
DX includes everything for Nucleo-F401RE:
...
| Flash command | `dx peripheral flash-nucleo` | Builds firmware, flashes via probe-rs |
...
From the dx repo root:

```bash
dx peripheral flash-nucleo
```

This builds `firmware/dx-nucleo` and runs `probe-rs run --chip STM32F401RETx`. The firmware runs immediately after flashing.
...
| 5 | `dx daemon` or `dx agent -m "Turn on LED"` |
...
- **GPIO commands ignored** — Check `path` in config matches your serial port. Run `dx peripheral list` to verify.
