# Οδηγός Ρυθμίσεων DX (config.toml)
...
Το DX ψάχνει για τις ρυθμίσεις με την εξής σειρά:
1. Στη διαδρομή που ορίζει η μεταβλητή `ZEROCLAW_WORKSPACE`.
2. Στο αρχείο `~/.dx/config.toml` (αυτή είναι η συνηθισμένη θέση).
...
- Αν αλλάξετε το αρχείο `config.toml`, πρέπει να κάνετε επανεκκίνηση το DX για να δει τις αλλαγές.
- Χρησιμοποιήστε την εντολή `dx doctor` για να βεβαιωθείτε ότι οι ρυθμίσεις σας είναι σωστές.
