...
Οδηγίες για την αρχική εγκατάσταση και την εξοικείωση με το οικοσύστημα του DX.
...
|:---|:---|
| Διαθέτω API Key και επιθυμώ άμεση ρύθμιση | `dx onboard --api-key sk-... --provider openrouter` |
| Επιθυμώ διαδραστική καθοδήγηση (Step-by-step) | `dx onboard --interactive` |
| Επιθυμώ μόνο τη ρύθμιση καναλιών επικοινωνίας | `dx onboard --channels-only` |
| Επιθυμώ πλήρη αντικατάσταση υπαρχουσών ρυθμίσεων | `dx onboard --force` |
...
- **Έλεγχος Υγείας**: Μετά την εγκατάσταση, εκτελέστε τις εντολές `dx status` και `dx doctor` για την επαλήθευση της σωστής λειτουργίας.
