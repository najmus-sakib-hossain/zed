...
Αυτός ο οδηγός περιγράφει τη διαδικασία διαμόρφωσης των καναλιών επικοινωνίας (Telegram, Discord, κ.λπ.) στο DX.
...
Οι ρυθμίσεις αποθηκεύονται στο αρχείο `~/.dx/config.toml`, στην ενότητα `[channels_config]`.
...
Σε πλατφόρμες όπως το Telegram και το Discord, μπορείτε να ελέγχετε το DX με εντολές:
...
Το DX υποστηρίζει την ανάγνωση εικόνων μέσω της σύνταξης:
`[IMAGE:<source>]`
...
5. **Επανεκκίνηση**: Κάθε αλλαγή στο `config.toml` απαιτεί επανεκκίνηση του `dx daemon`.

### Διαγνωστικά Μηνύματα (Logs)

Εκτελέστε το DX με αυξημένο επίπεδο καταγραφής:
```bash
RUST_LOG=info dx daemon
```
