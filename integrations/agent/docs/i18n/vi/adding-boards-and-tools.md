# Thêm Board và Tool — Hướng dẫn phần cứng DX

Hướng dẫn này giải thích cách thêm board phần cứng mới và tool tùy chỉnh vào DX.

## Bắt đầu nhanh: Thêm board qua CLI

```bash
# Thêm board (cập nhật ~/.dx/config.toml)
dx peripheral add nucleo-f401re /dev/ttyACM0
dx peripheral add arduino-uno /dev/cu.usbmodem12345
dx peripheral add rpi-gpio native   # cho Raspberry Pi GPIO (Linux)
...
Với feature `rag-pdf`, DX có thể lập chỉ mục file PDF:
...
2. **Thêm vào config** — `dx peripheral add my-board /dev/ttyUSB0`
...
| Lệnh | Mô tả |
|------|-------|
| `dx peripheral list` | Liệt kê các board đã cấu hình |
| `dx peripheral add <board> <path>` | Thêm board (ghi vào config) |
| `dx peripheral flash` | Nạp firmware Arduino |
| `dx peripheral flash-nucleo` | Nạp firmware Nucleo |
| `dx hardware discover` | Liệt kê thiết bị USB |
| `dx hardware info` | Thông tin chip qua probe-rs |
