# DX trên Nucleo-F401RE — Hướng dẫn từng bước

Chạy DX trên Mac hoặc Linux. Kết nối Nucleo-F401RE qua USB. Điều khiển GPIO (LED, các pin) qua Telegram hoặc CLI.

---

## Lấy thông tin board qua Telegram (Không cần nạp firmware)

DX có thể đọc thông tin chip từ Nucleo qua USB **mà không cần nạp firmware nào**. Nhắn tin cho Telegram bot của bạn:
...
DX bao gồm mọi thứ cần thiết cho Nucleo-F401RE:

| Thành phần | Vị trí | Mục đích |
|------------|--------|---------|
| Firmware | `firmware/dx-nucleo/` | Embassy Rust — USART2 (115200), gpio_read, gpio_write |
| Serial peripheral | `src/peripherals/serial.rs` | Giao thức JSON-over-serial (giống Arduino/ESP32) |
| Flash command | `dx peripheral flash-nucleo` | Build firmware, nạp qua probe-rs |
...
Từ thư mục gốc của repo dx:

```bash
dx peripheral flash-nucleo
```

Lệnh này build `firmware/dx-nucleo` và chạy `probe-rs run --chip STM32F401RETx`. Firmware chạy ngay sau khi nạp xong.
...
| 5 | `dx daemon` hoặc `dx agent -m "Turn on LED"` |
...
- **Lệnh GPIO bị bỏ qua** — Kiểm tra `path` trong config có khớp với serial port của bạn. Chạy `dx peripheral list` để xác nhận.
