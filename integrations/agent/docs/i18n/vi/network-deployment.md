# Triển khai mạng — DX trên Raspberry Pi và mạng nội bộ

Tài liệu này hướng dẫn triển khai DX trên Raspberry Pi hoặc host khác trong mạng nội bộ, với các channel Telegram và webhook tùy chọn.
...
|------|----------------------|----------|
| **Telegram polling** | Không | DX poll Telegram API; hoạt động từ bất kỳ đâu |
| **Matrix sync (kể cả E2EE)** | Không | DX sync qua Matrix client API; không cần webhook đến |
...
**Lưu ý:** Telegram, Discord và Slack dùng **long-polling** — DX thực hiện các request ra ngoài. Không cần port forwarding hoặc public IP.

---

## 2. DX trên Raspberry Pi
...
- Channel Telegram hoạt động: DX poll Telegram API (outbound)
...
```bash
dx daemon --host 0.0.0.0 --port 3000
```
...
2. Khởi động tunnel:
   ...
   ```
   Hoặc dùng `dx tunnel` (xem tài liệu tunnel).

3. DX sẽ từ chối `0.0.0.0` trừ khi `allow_public_bind = true` hoặc có tunnel đang hoạt động.
...
- DX gọi `https://api.telegram.org/bot{token}/getUpdates`
...
Chạy `dx daemon` — channel Telegram khởi động tự động.
...
- Chỉ chạy một instance runtime cho cùng token (khuyến nghị: service `dx daemon`).
...
- [ ] Chạy `dx daemon --host 127.0.0.1 --port 3000` (Telegram hoạt động không cần 0.0.0.0)
