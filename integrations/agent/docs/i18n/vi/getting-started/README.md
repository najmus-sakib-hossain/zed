...
| Tình huống | Lệnh |
|----------|---------|
| Có API key, muốn cài nhanh nhất | `dx onboard --api-key sk-... --provider openrouter` |
| Muốn được hướng dẫn từng bước | `dx onboard --interactive` |
| Đã có config, chỉ cần sửa kênh | `dx onboard --channels-only` |
...
- Thiết lập nhanh: `dx onboard --api-key "sk-..." --provider openrouter`
- Thiết lập tương tác: `dx onboard --interactive`
- Kiểm tra môi trường: `dx status` + `dx doctor`
