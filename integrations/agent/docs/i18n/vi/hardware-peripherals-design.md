# Thiết kế Hardware Peripherals — DX

DX cho phép các vi điều khiển (MCU) và máy tính nhúng (SBC) **phân tích lệnh ngôn ngữ tự nhiên theo thời gian thực**, tổng hợp code phù hợp với từng phần cứng, và thực thi tương tác với ngoại vi trực tiếp.

## 1. Tầm nhìn

**Mục tiêu:** DX đóng vai trò là AI agent có hiểu biết về phần cứng, cụ thể:
...
**Hình dung trực quan:** DX = bộ não hiểu phần cứng. Ngoại vi = tay chân mà nó điều khiển.
...
DX chạy **trực tiếp trên thiết bị**. Board khởi động server gRPC/nanoRPC và giao tiếp với ngoại vi ngay tại chỗ.

```
┌─────────────────────────────────────────────────────────────────────────────┐
│  DX on ESP32 / Raspberry Pi (Edge-Native)                             │
│                                                                             │
│  ┌─────────────┐    ┌──────────────┐    ┌─────────────────────────────────┐ │
...
2. DX truy xuất tài liệu theo board (ví dụ: bản đồ GPIO của ESP32)
...
DX chạy trên **máy chủ** và duy trì kết nối phần cứng tới thiết bị mục tiêu. Dùng cho phát triển, kiểm tra nội tâm, và nạp firmware.

```
┌─────────────────────┐                    ┌──────────────────────────────────┐
│  DX on Mac    │   USB / J-Link /   │  STM32 Nucleo-F401RE              │
...
2. DX nhận diện phần cứng đang kết nối (VID/PID, kiến trúc)
...
2. DX ghi/nạp firmware qua OpenOCD hoặc probe-rs
3. Xác nhận thành công

**Hoặc:**
1. DX tự phát hiện: *"STM32 Nucleo on /dev/ttyACM0, ARM Cortex-M4"*
...
|-----------|-------------|---------------|
| DX chạy trên | Thiết bị (ESP32, RPi) | Máy chủ (Mac, Linux) |
...
Máy chủ chạy DX; ngoại vi chạy firmware tối giản. JSON đơn giản qua serial.

### Chế độ B: RPi làm Host (Native GPIO)

DX trên Pi; GPIO qua rppal hoặc sysfs. Không cần firmware riêng.
...
dx agent --probe jlink

# Hardware introspection
dx hardware discover
dx hardware introspect /dev/ttyACM0
```
...
1. **Khởi động:** DX nạp config, đọc `peripherals.boards`.
...
Dành cho RPC có kiểu dữ liệu, độ trễ thấp giữa DX và các peripheral:
...
- **dx-firmware** hoặc **dx-peripheral** — một crate/workspace riêng biệt.
...
- Người dùng nạp lên board; DX kết nối và tự phát hiện khả năng.

## 9. Các giai đoạn triển khai

### Phase 1: Skeleton ✅ (Hoàn thành)

- [x] Thêm trait `Peripheral`, config schema, CLI (`dx peripheral list/add`)
- [x] Thêm flag `--peripheral` cho agent
- [x] Ghi tài liệu vào AGENTS.md

### Phase 2: Host-Mediated — Phát hiện phần cứng ✅ (Hoàn thành)

- [x] `dx hardware discover`: liệt kê thiết bị USB (VID/PID)
- [x] Board registry: ánh xạ VID/PID → kiến trúc, tên (ví dụ: Nucleo-F401RE)
- [x] `dx hardware introspect <path>`: memory map, danh sách peripheral
...
- [x] DX trên Raspberry Pi (native GPIO qua rppal)
...
- [x] Crate firmware `dx-esp32` (`firmware/dx-esp32`) — GPIO qua UART
- [x] ESP32 trong hardware registry (CH340 VID/PID)
- [ ] DX *chạy trực tiếp trên* ESP32 (WiFi + LLM, edge-native) — tương lai
- [ ] Thực thi Wasm hoặc dựa trên template cho logic do LLM tạo ra

**Cách dùng:** Nạp `firmware/dx-esp32` vào ESP32, thêm `board = "esp32"`, `transport = "serial"`, `path = "/dev/ttyUSB0"` vào config.
...
- Chạy DX đầy đủ *trực tiếp trên* STM32 bare-metal (không có WiFi, RAM hạn chế) — dùng Host-Mediated thay thế
...
> *"Các board như ESP, Raspberry Pi, hoặc các board có WiFi có thể kết nối với LLM (Gemini hoặc mã nguồn mở). DX chạy trên thiết bị, tạo gRPC riêng, khởi động nó, và giao tiếp với ngoại vi. Người dùng hỏi qua WhatsApp: 'di chuyển cánh tay X' hoặc 'bật LED'. DX lấy tài liệu chính xác, viết code, thực thi, lưu trữ tối ưu, chạy, và bật LED — tất cả trên board phát triển.*
>
> *Với STM Nucleo kết nối qua USB/J-Link/Aardvark vào Mac: DX từ Mac truy cập phần cứng, cài đặt hoặc ghi những gì cần thiết lên thiết bị, và trả kết quả. Ví dụ: 'Hey DX, những địa chỉ khả dụng/đọc được trên thiết bị USB này là gì?' Nó có thể tự tìm ra thiết bị nào đang kết nối ở đâu và đưa ra gợi ý."*
