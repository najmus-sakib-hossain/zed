# DX trên Arduino Uno Q — Hướng dẫn từng bước

Chạy DX trên phía Linux của Arduino Uno Q. Telegram hoạt động qua WiFi; điều khiển GPIO dùng Bridge (yêu cầu một ứng dụng App Lab tối giản).

---

## Những gì đã có sẵn (Không cần thay đổi code)

DX bao gồm mọi thứ cần thiết cho Arduino Uno Q. **Clone repo và làm theo hướng dẫn này — không cần patch hay code tùy chỉnh nào.**

| Thành phần | Vị trí | Mục đích |
|------------|--------|---------|
| Bridge app | `firmware/dx-uno-q-bridge/` | MCU sketch + Python socket server (port 9999) cho GPIO |
| Bridge tools | `src/peripherals/uno_q_bridge.rs` | Tool `gpio_read` / `gpio_write` giao tiếp với Bridge qua TCP |
| Setup command | `src/peripherals/uno_q_setup.rs` | `dx peripheral setup-uno-q` triển khai Bridge qua scp + arduino-app-cli |
...
# Clone dx (hoặc scp project của bạn)
git clone https://github.com/theonlyhennygod/dx.git
cd dx
...
# Copy sang Uno Q
scp target/aarch64-unknown-linux-gnu/release/dx arduino@<UNO_Q_IP>:~/
ssh arduino@<UNO_Q_IP> "sudo mv ~/dx /usr/local/bin/"
```
...
# Cấu hình nhanh
dx onboard --api-key YOUR_OPENROUTER_KEY --provider openrouter

# Hoặc tạo config thủ công
mkdir -p ~/.dx/workspace
nano ~/.dx/config.toml
```
...
## Phase 4: Chạy DX Daemon

```bash
ssh arduino@<UNO_Q_IP>

# Chạy daemon (Telegram polling hoạt động qua WiFi)
dx daemon --host 127.0.0.1 --port 3000
```

**Tại bước này:** Telegram chat hoạt động. Gửi tin nhắn tới bot — DX phản hồi. Chưa có GPIO.

---

## Phase 5: GPIO qua Bridge (DX xử lý tự động)

DX bao gồm Bridge app và setup command.

### 5.1 Triển khai Bridge App

**Từ Mac** (với repo dx):
```bash
dx peripheral setup-uno-q --host 192.168.0.48
```

**Từ Uno Q** (đã SSH vào):
```bash
dx peripheral setup-uno-q
```

Lệnh này copy Bridge app vào `~/ArduinoApps/dx-uno-q-bridge` và khởi động nó.
...
Giờ khi bạn nhắn tin cho Telegram bot *"Turn on the LED"* hoặc *"Set pin 13 high"*, DX dùng `gpio_write` qua Bridge.
...
| 5 | `git clone https://github.com/theonlyhennygod/dx.git && cd dx` |
| 6 | `cargo build --release --no-default-features` |
| 7 | `dx onboard --api-key KEY --provider openrouter` |
| 8 | Chỉnh sửa `~/.dx/config.toml` (thêm Telegram bot_token) |
| 9 | `dx daemon --host 127.0.0.1 --port 3000` |
...
- **"command not found: dx"** — Dùng đường dẫn đầy đủ: `/usr/local/bin/dx` hoặc đảm bảo `~/.cargo/bin` nằm trong PATH.
...
- **Lệnh GPIO bị bỏ qua** — Đảm bảo Bridge app đang chạy (`dx peripheral setup-uno-q` triển khai và khởi động nó). Config phải có `board = "arduino-uno-q"` và `transport = "bridge"`.
- **LLM provider (GLM/Zhipu)** — Dùng `default_provider = "glm"` hoặc `"zhipu"` với `GLM_API_KEY` trong env hoặc config. DX dùng endpoint v4 chính xác.
