# Network Deployment — DX on Raspberry Pi and Local Network

This document covers deploying DX on a Raspberry Pi or other host on your local network, with Telegram and optional webhook channels.
...
|------|----------------------|----------|
| **Telegram polling** | No | DX polls Telegram API; works from anywhere |
| **Matrix sync (including E2EE)** | No | DX syncs via Matrix client API; no inbound webhook required |
...
**Key:** Telegram, Discord, Slack, and Nostr use **outbound connections** — DX connects to external servers/relays. No port forwarding or public IP required.

---

## 2. DX on Raspberry Pi
...
- Telegram channel works: DX polls Telegram API (outbound)
...
2. Start a tunnel:
   ...
   ```
   Or use `dx tunnel` (see tunnel docs).

3. DX will refuse `0.0.0.0` unless `allow_public_bind = true` or a tunnel is active.
...
- DX calls `https://api.telegram.org/bot{token}/getUpdates`
...
Run `dx daemon` — Telegram channel starts automatically.
...
- Keep one runtime instance for the same token (recommended: `dx daemon` service).
...
- [ ] Run `dx daemon --host 127.0.0.1 --port 42617` (Telegram works without 0.0.0.0)
...
DX supports OpenRC for Alpine Linux and other distributions using the OpenRC init system. OpenRC services run **system-wide** and require root/sudo.

### 7.1 Prerequisites

- Alpine Linux (or another OpenRC-based distro)
- Root or sudo access
- A dedicated `dx` system user (created during install)
...
This creates:
- Init script: `/etc/init.d/dx`
- Config directory: `/etc/dx/`
- Log directory: `/var/log/dx/`

### 7.3 Configuration

Manual config copy is usually not required.

`sudo dx service install` automatically prepares `/etc/dx`, migrates existing runtime state from your user setup when available, and sets ownership/permissions for the `dx` service user.

If no prior runtime state is available to migrate, create `/etc/dx/config.toml` before starting the service.
...
| Command | Description |
|---------|-------------|
| `sudo rc-service dx start` | Start the daemon |
| `sudo rc-service dx stop` | Stop the daemon |
| `sudo rc-service dx status` | Check service status |
| `sudo rc-service dx restart` | Restart the daemon |
| `sudo dx service status` | DX status wrapper (uses `/etc/dx` config) |
...
| Log | Path |
|-----|------|
| Access/stdout | `/var/log/dx/access.log` |
| Errors/stderr | `/var/log/dx/error.log` |
...
sudo rc-service dx stop
sudo rc-update del dx default

# Remove init script
sudo dx service uninstall
```
...
- The service runs as the `dx:dx` user (least privilege)
- Config must be at `/etc/dx/config.toml` (explicit path in init script)
- If the `dx` user does not exist, install will fail with instructions to create it

### 7.9 Checklist: Alpine/OpenRC Deployment

- [ ] Install: `sudo dx service install`
- [ ] Enable: `sudo rc-update add dx default`
- [ ] Start: `sudo rc-service dx start`
- [ ] Verify: `sudo rc-service dx status`
- [ ] Check logs: `/var/log/dx/error.log`
