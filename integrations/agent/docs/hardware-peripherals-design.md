# Hardware Peripherals Design — DX

DX enables microcontrollers (MCUs) and Single Board Computers (SBCs) to **dynamically interpret natural language commands**, generate hardware-specific code, and execute peripheral interactions in real-time.

## 1. Vision

**Goal:** DX acts as a hardware-aware AI agent that:
...
**Mental model:** DX = brain that understands hardware. Peripherals = arms and legs it controls.
...
DX runs **directly on the device**. The board spins up a gRPC/nanoRPC server and communicates with peripherals locally.
...
2. DX fetches board-specific docs (e.g. ESP32 GPIO mapping)
...
DX runs on the **host** and maintains a hardware-aware link to the target. Used for development, introspection, and flashing.
...
2. DX identifies connected hardware (VID/PID, architecture)
...
2. DX writes/flashes via OpenOCD or probe-rs
3. Confirms success

**Or:**
1. DX auto-discovers: *"STM32 Nucleo on /dev/ttyACM0, ARM Cortex-M4"*
...
|------------------|--------------------------------|----------------------------------|
| DX runs on | Device (ESP32, RPi)           | Host (Mac, Linux)                |
...
Host runs DX; peripheral runs minimal firmware. Simple JSON over serial.

### Mode B: RPi as Host (Native GPIO)

DX on Pi; GPIO via rppal or sysfs. No separate firmware.
...
1. **Startup:** DX loads config, sees `peripherals.boards`.
...
For low-latency, typed RPC between DX and peripherals:
...
- **dx-firmware** or **dx-peripheral** — a separate crate/workspace.
...
- User flashes this to the board; DX connects and discovers capabilities.

## 9. Implementation Phases

### Phase 1: Skeleton ✅ (Done)

- [x] Add `Peripheral` trait, config schema, CLI (`dx peripheral list/add`)
- [x] Add `--peripheral` flag to agent
- [x] Document in AGENTS.md

### Phase 2: Host-Mediated — Hardware Discovery ✅ (Done)

- [x] `dx hardware discover`: enumerate USB devices (VID/PID)
- [x] Board registry: map VID/PID → architecture, name (e.g. Nucleo-F401RE)
- [x] `dx hardware introspect <path>`: memory map, peripheral list
...
- [x] DX on Raspberry Pi (native GPIO via rppal)
...
- [x] `dx-esp32` firmware crate (`firmware/dx-esp32`) — GPIO over UART
- [x] ESP32 in hardware registry (CH340 VID/PID)
- [ ] DX *on* ESP32 (WiFi + LLM, edge-native) — future
- [ ] Wasm or template-based execution for LLM-generated logic

**Usage:** Flash `firmware/dx-esp32` to ESP32, add `board = "esp32"`, `transport = "serial"`, `path = "/dev/ttyUSB0"` to config.
...
- Running full DX *on* bare STM32 (no WiFi, limited RAM) — use Host-Mediated instead
...
> *"Boards like ESP, Raspberry Pi, or boards with WiFi can connect to an LLM (Gemini or open-source). DX runs on the device, creates its own gRPC, spins it up, and communicates with peripherals. User asks via WhatsApp: 'move X arm' or 'turn on LED'. DX gets accurate documentation, writes code, executes it, stores it optimally, runs it, and turns on the LED — all on the development board.*
>
> *For STM Nucleo connected via USB/J-Link/Aardvark to my Mac: DX from my Mac accesses the hardware, installs or writes what it wants on the device, and returns the result. Example: 'Hey DX, what are the available/readable addresses on this USB device?' It can figure out what's connected where and suggest."*
