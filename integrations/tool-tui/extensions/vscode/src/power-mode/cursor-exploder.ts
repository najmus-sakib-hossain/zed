import type { BackgroundMode, ExplosionConfig, ExplosionOrder, GifMode, Plugin, PowermodeChangeTextDocumentEventData, ThemeConfig } from './types'
import * as vscode from 'vscode'
import { getConfigValue } from './config'
import { CSS_LEFT, CSS_TOP } from './constants'
...
export class CursorExploder implements Plugin {
  ...
  public onDidChangeTextDocument = (data: PowermodeChangeTextDocumentEventData, event: vscode.TextDocumentChangeEvent) => {
    console.log('DX Explosion: CursorExploder.onDidChangeTextDocument called, explosions.enable:', this.config['explosions.enable'], 'isPowermodeActive:', data.isPowermodeActive)
    
    if (!this.config['explosions.enable'] || !data.isPowermodeActive) {
      console.log('DX Explosion: CursorExploder skipping - not enabled or not active')
      return
    }

    console.log('DX Explosion: CursorExploder creating explosion!')
  ...
  public onDidChangeConfiguration = (_config: vscode.WorkspaceConfiguration) => {
    const newConfig: ExplosionConfig = {
      'explosions.customExplosions': getConfigValue<string[]>('explosions.customExplosions', this.themeConfig),
      'explosions.enable': getConfigValue<boolean>('explosions.enable', this.themeConfig) ?? true, // DX: Default true
      'explosions.maxExplosions': getConfigValue<number>('explosions.maxExplosions', this.themeConfig),
      'explosions.size': getConfigValue<number>('explosions.size', this.themeConfig),
      'explosions.frequency': getConfigValue<number>('explosions.frequency', this.themeConfig),
      'explosions.offset': getConfigValue<number>('explosions.offset', this.themeConfig),
      'explosions.explosionOrder': getConfigValue<ExplosionOrder>('explosions.explosionOrder', this.themeConfig),
      'explosions.duration': getConfigValue<number>('explosions.duration', this.themeConfig),
      'explosions.backgroundMode': getConfigValue<BackgroundMode>('explosions.backgroundMode', this.themeConfig),
      'explosions.gifMode': getConfigValue<GifMode>('explosions.gifMode', this.themeConfig),
      'explosions.customCss': getConfigValue<Record<string, string>>('explosions.customCss', this.themeConfig),
    }

    console.log('DX Explosion: CursorExploder config updated, explosions.enable:', newConfig['explosions.enable'])

    let changed = false
    Object.keys(newConfig).forEach((key) => {
      const field = key as keyof ExplosionConfig
  ...
  private getExplosionDecoration = (position: vscode.Position): vscode.TextEditorDecorationType | null => {
    const explosions = this.config['explosions.customExplosions']
  ...
  private pickExplosion(explosions: string[]): string | null {
    if (!explosions)
      return null
   ...
   */
  private createExplosionDecorationType = (explosion: string, _editorPosition: vscode.Position): vscode.TextEditorDecorationType => {
  ...
  private getMaskCssSettings(explosion: string): Record<string, string | number> {
    return {
  ...
  private objectToCssString(settings: Record<string, string | number>): string {
    let value = ''
   ...
   *
   * @param editor represents an editor that is attached to a document
   * @param left place the decoration to the left or the right of the cursor
   */
