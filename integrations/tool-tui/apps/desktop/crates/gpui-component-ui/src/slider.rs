...
impl Slider {
    ...
    fn render_thumb(
        ...
        div()
            .id(id)
            .absolute()
            .when(axis.is_horizontal(), |this| this.top(px(-5.)).left(start).ml(-px(8.)))
            .when(axis.is_vertical(), |this| this.bottom(start).left(px(-5.)).mb(-px(8.)))
            .flex()
            .items_center()
            .justify_center()
            .flex_shrink_0()
...
impl RenderOnce for Slider {
    fn render(self, window: &mut Window, cx: &mut gpui::App) -> impl IntoElement {
        ...
        let thumb_color = self.style.text.color.unwrap_or_else(|| cx.theme().slider_thumb);
