...
impl Root {
    ...
    pub fn render_notification_layer(
        ...
        Some(
            div()
                .absolute()
                .when(matches!(placement, Anchor::TopRight), |this| this.top_0().right_0())
                .when(matches!(placement, Anchor::TopLeft), |this| this.top_0().left_0())
                .when(matches!(placement, Anchor::TopCenter), |this| this.top_0().mx_auto())
                .when(matches!(placement, Anchor::BottomRight), |this| this.bottom_0().right_0())
                .when(matches!(placement, Anchor::BottomLeft), |this| this.bottom_0().left_0())
                .when(matches!(placement, Anchor::BottomCenter), |this| this.bottom_0().mx_auto())
        ...
        F: Fn(Dialog, &mut Window, &mut App) -> Dialog + 'static,
    {
        ...
        self.active_dialogs
            .push(ActiveDialog::new(focus_handle, previous_focused_handle, build));
        cx.notify();
    }
    ...
    pub fn close_all_dialogs(&mut self, window: &mut Window, cx: &mut Context<'_, Root>) {
        self.focused_input = None;
        let previous_focused_handle =
            self.active_dialogs.first().and_then(|d| d.previous_focused_handle.clone());
    ...
    pub fn push_notification(
        ...
        self.notification.update(cx, |view, cx| view.push(note, window, cx));
        cx.notify();
    }
    ...
    pub fn clear_notifications(&mut self, window: &mut Window, cx: &mut Context<'_, Root>) {
        self.notification.update(cx, |view, cx| view.clear(window, cx));
        cx.notify();
    }
