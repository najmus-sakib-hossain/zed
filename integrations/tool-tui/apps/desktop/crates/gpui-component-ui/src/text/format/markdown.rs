...
fn parse_paragraph(paragraph: &mut Paragraph, node: &mdast::Node, cx: &mut NodeContext) -> String {
    ...
    match node {
        ...
        Node::Html(val) => match super::html::parse(&val.value, cx) {
            Ok(el) => {
                if el.blocks.first().map(|node| node.is_break()).unwrap_or(false) {
                    text = "\n".to_owned();
...
fn ast_to_document(
    ...
    let blocks = root.children.into_iter().map(|c| ast_to_node(c, cx, highlight_theme)).collect();
    ParsedDocument {
...
fn ast_to_node(
    ...
    match value {
        ...
        Node::Blockquote(val) => {
            let children =
                val.children.into_iter().map(|c| ast_to_node(c, cx, highlight_theme)).collect();
        ...
        Node::List(list) => {
            let children =
                list.children.into_iter().map(|c| ast_to_node(c, cx, highlight_theme)).collect();
            BlockNode::List {
