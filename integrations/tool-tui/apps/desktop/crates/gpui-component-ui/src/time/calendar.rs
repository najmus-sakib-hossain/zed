...
impl CalendarState {
    ...
    pub fn set_date(&mut self, date: impl Into<Date>, _: &mut Window, cx: &mut Context<Self>) {
        ...
        let invalid =
            self.disabled_matcher.as_ref().map_or(false, |matcher| matcher.is_match(&date));
        ...
        h_flex()
            ...
            .child(
                Button::new("prev")
                    ...
                    .when(view_mode.is_year(), |this| {
                        this.when(!state.has_prev_year_page(), |this| this.disabled(true)).on_click(
            ...
            .child(
                Button::new("next")
                    ...
                    .when(view_mode.is_year(), |this| {
                        this.when(!state.has_next_year_page(), |this| this.disabled(true)).on_click(
            ...
            .children(state.days().chunks(5).enumerate().map(|(offset_month, days)| {
                v_flex()
                    ...
                    .child(h_flex().gap_0p5().justify_between().children(
                        weeks.iter().map(|week| self.render_week(week.clone(), window, cx)),
                    ))
                    .children(days.iter().map(|week| {
                        h_flex().gap_0p5().justify_between().children(
                            week.iter().map(|d| self.render_day(d, offset_month, window, cx)),
                        )
                    }))
            }))
    }
