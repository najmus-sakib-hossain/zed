...
impl HslaSliders {
    fn new(cx: &mut App) -> Self {
        Self {
            hue: cx.new(|_| SliderState::new().min(0.).max(1.).step(0.01).default_value(0.)),
            saturation: cx.new(|_| SliderState::new().min(0.).max(1.).step(0.01).default_value(0.)),
            lightness: cx.new(|_| SliderState::new().min(0.).max(1.).step(0.01).default_value(0.)),
            alpha: cx.new(|_| SliderState::new().min(0.).max(1.).step(0.01).default_value(0.)),
        }
    }
...
impl ColorPickerState {
    ...
    pub fn new(window: &mut Window, cx: &mut Context<Self>) -> Self {
        ...
        let mut _subscriptions = vec![
            cx.subscribe_in(&state, window, |this, state, ev: &InputEvent, window, cx| match ev {
                InputEvent::Change => {
            ...
            cx.subscribe_in(&hsla_sliders.hue, window, |this, _, _: &SliderEvent, window, cx| {
            ...
            cx.subscribe_in(&hsla_sliders.alpha, window, |this, _, _: &SliderEvent, window, cx| {
                let color = this.hsla_sliders.read(cx);
                this.update_value_from_slider(color, true, window, cx);
            }),
        ];
...
impl ColorPicker {
    ...
    fn render_colors(&self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        let (slider_color, hovered_color) = {
            ...
            let slider_color =
                state.hovered_color.or(state.value).unwrap_or_else(|| hsla(0., 0., 0., 1.));
        ...
        v_flex()
            ...
            .child(
                TabBar::new("mode")
                    ...
                    .on_click(window.listener_for(&self.state, |state, ix: &usize, _, cx| {
                        state.active_tab = *ix;
                        cx.notify();
                    }))
            ...
            .child(match active_tab {
                ...
                _ => self.render_slider_tab_panel(slider_color, cx).into_any_element(),
            })
    ...
    fn render_palette_panel(&self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        v_flex()
            ...
            .child(h_flex().gap_1().children(
                featured_colors.iter().map(|color| self.render_item(*color, true, window, cx)),
            ))
            .child(Divider::horizontal())
            .child(v_flex().gap_1().children(color_palettes().iter().map(|sub_colors| {
                h_flex().gap_1().children(
                    sub_colors.iter().rev().map(|color| self.render_item(*color, true, window, cx)),
                )
            })))
    }
                ...
                h_flex()
                    ...
                    .child(
                        div()
                            ...
                            .child(
                                Slider::new(&hsla_sliders.hue).flex_1().bg(cx.theme().transparent),
                            ),
                    )
                    .child(
                        div()
    ...
    fn render_slider_track(&self, colors: Vec<Hsla>, _: &App) -> impl IntoElement {
        h_flex()
            ...
            .h_2_5()
            .overflow_hidden()
            .children(colors.into_iter().map(|color| div().flex_1().h_full().bg(color)))
    }
    ...
    fn render_slider_track_gradient(&self, start: Hsla, end: Hsla, _: &App) -> impl IntoElement {
        div()
            ...
            .h_2_5()
            .overflow_hidden()
            .bg(linear_gradient(90., linear_color_stop(start, 0.), linear_color_stop(end, 1.)))
    }
}
        ...
        div()
            ...
            .child(
                Popover::new("popover")
                    .open(state.open)
                    .w_72()
                    .on_open_change(window.listener_for(&self.state, |this, open: &bool, _, cx| {
                        this.open = *open;
                        cx.notify();
                    }))
                    .trigger(
