...
impl DockArea {
    ...
    pub fn add_panel(
        ...
        match placement {
            ...
            DockPlacement::Center => {
                self.center.add_panel(panel, &cx.entity().downgrade(), bounds, window, cx);
            }
        }
    }
    ...
    pub fn update_toggle_button_tab_panels(&mut self, _: &mut Window, cx: &mut Context<Self>) {
        ...
        self.toggle_button_panels.left =
            self.center.left_top_tab_panel(cx).map(|view| view.entity_id());

        // Right toggle button
        self.toggle_button_panels.right =
            self.center.right_top_tab_panel(cx).map(|view| view.entity_id());
