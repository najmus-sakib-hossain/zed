...
impl SyntaxHighlighter {
    ...
    fn parse_combined_injections(&mut self, tree: &Tree) {
        ...
        while let Some(query_match) = matches.next() {
            ...
            {
                language_name = prop.value.as_ref().map(|v| SharedString::from(v.to_string()));
            }
        ...
        for (language_name, ranges) in combined_ranges {
            ...
            let old_tree = self.injection_layers.get(&language_name).map(|layer| &layer.tree);
            ...
            self.injection_layers.insert(language_name, InjectionLayer { tree: new_tree });
        }
    }
...
"#;
        ...
        for tag in closing_tags {
            ...
            assert!(has_highlight, "closing tag {} at byte {} should be highlighted", tag, pos);
        }
    }
