...
impl ResizableState {
    ...
    fn resize_panel(&mut self, ix: usize, size: Pixels, _: &mut Window, cx: &mut Context<Self>) {
        ...
        // Can't resize the last panel (no right panel to take from)
        ...
        let right_ix = ix + 1;
        let right_size_range = self.panel_size_range(right_ix);

        if is_expand {
            // Expanding left panel - take space from right panel
            let changed = new_size - old_sizes[ix];
            let available_size = (old_sizes[right_ix] - right_size_range.start).max(px(0.));
            let actual_change = changed.min(available_size);

            if actual_change > px(0.) {
                new_sizes[ix] = old_sizes[ix] + actual_change;
                new_sizes[right_ix] =
                    (old_sizes[right_ix] - actual_change).max(right_size_range.start);
            }
        } else {
            // Shrinking left panel - give space to right panel
            let changed = old_sizes[ix] - new_size;
            let new_right_size = old_sizes[right_ix] + changed;

            // Clamp right panel to its max size
            let clamped_right_size = new_right_size.min(right_size_range.end);
            let actual_change = clamped_right_size - old_sizes[right_ix];

            new_sizes[ix] = old_sizes[ix] - actual_change;
            new_sizes[right_ix] = clamped_right_size;
        }

        // Update only the affected panels
        self.panels[ix].size = Some(new_sizes[ix]);
        self.sizes[ix] = new_sizes[ix];
        self.panels[right_ix].size = Some(new_sizes[right_ix]);
        self.sizes[right_ix] = new_sizes[right_ix];
