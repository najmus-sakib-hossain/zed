...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") {
    await loadPresence(host as unknown as DxApp);
  }
  if (host.tab === "sessions") {
    await loadSessions(host as unknown as DxApp);
  }
  if (host.tab === "cron") {
    await loadCron(host);
  }
  if (host.tab === "skills") {
    await loadSkills(host as unknown as DxApp);
  }
  if (host.tab === "agents") {
    const app = host as unknown as DxApp;
  ...
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as DxApp);
    await loadDevices(host as unknown as DxApp);
    await loadConfig(host as unknown as DxApp);
    await loadExecApprovals(host as unknown as DxApp);
  }
  if (host.tab === "chat") {
  ...
  if (host.tab === "config") {
    await loadConfigSchema(host as unknown as DxApp);
    await loadConfig(host as unknown as DxApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as DxApp);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as DxApp, { reset: true });
...
export async function loadOverview(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as DxApp, false),
    loadPresence(host as unknown as DxApp),
    loadSessions(host as unknown as DxApp),
    loadCronStatus(host as unknown as DxApp),
    loadDebug(host as unknown as DxApp),
  ]);
}

export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as DxApp, true),
    loadConfigSchema(host as unknown as DxApp),
    loadConfig(host as unknown as DxApp),
  ]);
}
...
export async function loadCron(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as DxApp, false),
    loadCronStatus(host as unknown as DxApp),
    loadCronJobs(host as unknown as DxApp),
  ]);
}
