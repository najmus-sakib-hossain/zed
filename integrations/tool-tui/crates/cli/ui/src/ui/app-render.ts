          ...
          state.tab === "overview"
            ? renderOverview({
                ...
                onSessionKeyChange: (next) => {
                  ...
                  state.chatMessage = "";
                  (state as unknown as DxApp).resetToolStream();
                  state.applySettings({
                    ...state.settings,
                    sessionKey: next,
          ...
          state.tab === "chat"
            ? renderChat({
                ...
                onSessionKeyChange: (next) => {
                  ...
                  (state as unknown as DxApp).chatStreamStartedAt = null;
                  state.chatQueue = [];
                  (state as unknown as DxApp).resetToolStream();
                  (state as unknown as DxApp).resetChatScroll();
                  state.applySettings({
                    ...state.settings,
                    sessionKey: next,
                    lastActiveSessionKey: next,
                ...
                onChatScroll: (event) => (state as unknown as DxApp).handleChatScroll(event),
                onDraftChange: (next) => (state.chatMessage = next),
                attachments: state.chatAttachments,
                onAttachmentsChange: (next) => (state.chatAttachments = next),
                onSend: () => (state as unknown as DxApp).handleSendChat(),
                canAbort: Boolean(state.chatRunId),
                onAbort: () => void (state as unknown as DxApp).handleAbortChat(),
                onQueueRemove: (id) => (state as unknown as DxApp).removeQueuedMessage(id),
                onNewSession: () =>
                  (state as unknown as DxApp).handleSendChat("/new", { restoreDraft: true }),
                showNewMessages: state.chatNewMessagesBelow,
                onScrollToBottom: () => state.scrollToBottom(),
                // Sidebar props for tool output viewing
                sidebarOpen: (state as unknown as DxApp).sidebarOpen,
                sidebarContent: (state as unknown as DxApp).sidebarContent,
                sidebarError: (state as unknown as DxApp).sidebarError,
                splitRatio: (state as unknown as DxApp).splitRatio,
                onOpenSidebar: (content: string) =>
                  (state as unknown as DxApp).handleOpenSidebar(content),
                onCloseSidebar: () => (state as unknown as DxApp).handleCloseSidebar(),
                onSplitRatioChange: (ratio: number) =>
                  (state as unknown as DxApp).handleSplitRatioChange(ratio),
        ...
        ${
          state.tab === "config"
            ? renderConfig({
                ...
                searchQuery: (state as unknown as DxApp).configSearchQuery,
                activeSection: (state as unknown as DxApp).configActiveSection,
                activeSubsection: (state as unknown as DxApp).configActiveSubsection,
                onRawChange: (next) => {
                  state.configRaw = next;
                },
                onFormModeChange: (mode) => (state.configFormMode = mode),
                onFormPatch: (path, value) =>
                  updateConfigFormValue(state as unknown as DxApp, path, value),
                onSearchChange: (query) =>
                  ((state as unknown as DxApp).configSearchQuery = query),
                onSectionChange: (section) => {
                  (state as unknown as DxApp).configActiveSection = section;
                  (state as unknown as DxApp).configActiveSubsection = null;
                },
                onSubsectionChange: (section) =>
                  ((state as unknown as DxApp).configActiveSubsection = section),
                onReload: () => loadConfig(state as unknown as DxApp),
                onSave: () => saveConfig(state as unknown as DxApp),
                onApply: () => applyConfig(state as unknown as DxApp),
                onUpdate: () => runUpdate(state as unknown as DxApp),
              })
            : nothing
        }
          ...
          state.tab === "logs"
            ? renderLogs({
                ...
                onExport: (lines, label) =>
                  (state as unknown as DxApp).exportLogs(lines, label),
                onScroll: (event) => (state as unknown as DxApp).handleLogsScroll(event),
              })
            : nothing
        }
      </main>
