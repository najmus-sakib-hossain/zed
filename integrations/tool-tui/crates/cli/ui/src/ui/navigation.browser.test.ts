...
const originalConnect = DxApp.prototype.connect;

function mountApp(pathname: string) {
  window.history.replaceState({}, "", pathname);
  const app = document.createElement("dx-app") as DxApp;
...
afterEach(() => {
  DxApp.prototype.connect = originalConnect;
  window.__OPENCLAW_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/dx/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/dx");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/dx/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__OPENCLAW_CONTROL_UI_BASE_PATH__ = "/dx";
    const app = mountApp("/dx/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/dx");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/dx/sessions");
  });
