...
IMAGE_NAME="${OPENCLAW_IMAGE:-${CLAWDBOT_IMAGE:-dx:local}}"
CONFIG_DIR="${OPENCLAW_CONFIG_DIR:-${CLAWDBOT_CONFIG_DIR:-$HOME/.dx}}"
WORKSPACE_DIR="${OPENCLAW_WORKSPACE_DIR:-${CLAWDBOT_WORKSPACE_DIR:-$HOME/.dx/workspace}}"
...
docker run --rm -t \
  ...
  -v "$CONFIG_DIR":/home/node/.dx \
  -v "$WORKSPACE_DIR":/home/node/.dx/workspace \
  "${PROFILE_MOUNT[@]}" \
  "$IMAGE_NAME" \
