#!/bin/bash
# Setup Dx Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'dx-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Dx message: Send warning via Dx itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/dx-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/dx-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/dx-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now dx-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status dx-auth-monitor.timer"
echo ""
echo "Quick commands:"
