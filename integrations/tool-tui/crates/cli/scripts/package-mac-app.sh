...
# Build and bundle Dx into a minimal .app we can open.
# Outputs to dist/Dx.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Dx.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Dx"
BUNDLE_ID="${BUNDLE_ID:-ai.dx.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/dx/dx/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Dx/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :DxBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :DxGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Dx"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Dx"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Dx"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Dx" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Dx/Resources/Dx.icns" "$APP_ROOT/Contents/Resources/Dx.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Dx/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying DxKit resources"
OPENCLAWKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/DxKit_DxKit.bundle"
if [ -d "$OPENCLAWKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/DxKit_DxKit.bundle"
  cp -R "$OPENCLAWKIT_BUNDLE" "$APP_ROOT/Contents/Resources/DxKit_DxKit.bundle"
else
  echo "WARN: DxKit resource bundle not found at $OPENCLAWKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Dx"
killall -q Dx 2>/dev/null || true
