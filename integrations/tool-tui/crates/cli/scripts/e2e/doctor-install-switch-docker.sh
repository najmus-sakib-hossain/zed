...
IMAGE_NAME="dx-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/dx-bin:$PATH"
  mkdir -p /tmp/dx-bin

  cat > /tmp/dx-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/dx-bin/systemctl

  cat > /tmp/dx-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
LOGINCTL
  ...
  npm_bin="/tmp/npm-prefix/bin/dx"
  npm_entry="/tmp/npm-prefix/lib/node_modules/dx/dist/index.js"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/dx-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/dx-gateway.service"
    if [ ! -f "$unit_path" ]; then
