...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  home_dir=$(mktemp -d "/tmp/dx-plugins-e2e.XXXXXX")
  export HOME="$home_dir"
  mkdir -p "$HOME/.dx/extensions/demo-plugin"

  cat > "$HOME/.dx/extensions/demo-plugin/index.js" <<'"'"'JS'"'"'
module.exports = {
  id: "demo-plugin",
  name: "Demo Plugin",
...
JS
  cat > "$HOME/.dx/extensions/demo-plugin/dx.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin",
  "configSchema": {
    "type": "object",
...
NODE
  ...
  pack_dir="$(mktemp -d "/tmp/dx-plugin-pack.XXXXXX")"
  mkdir -p "$pack_dir/package"
  cat > "$pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@dx/demo-plugin-tgz",
  "version": "0.0.1",
  "dx": { "extensions": ["./index.js"] }
}
JSON
...
JS
  cat > "$pack_dir/package/dx.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-tgz",
  "configSchema": {
    "type": "object",
...
NODE
  ...
  dir_plugin="$(mktemp -d "/tmp/dx-plugin-dir.XXXXXX")"
  cat > "$dir_plugin/package.json" <<'"'"'JSON'"'"'
{
  "name": "@dx/demo-plugin-dir",
  "version": "0.0.1",
  "dx": { "extensions": ["./index.js"] }
}
JSON
...
JS
  cat > "$dir_plugin/dx.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-dir",
  "configSchema": {
    "type": "object",
...
NODE
  ...
  file_pack_dir="$(mktemp -d "/tmp/dx-plugin-filepack.XXXXXX")"
  mkdir -p "$file_pack_dir/package"
  cat > "$file_pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@dx/demo-plugin-file",
  "version": "0.0.1",
  "dx": { "extensions": ["./index.js"] }
}
JSON
...
JS
  cat > "$file_pack_dir/package/dx.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-file",
  "configSchema": {
    "type": "object",
