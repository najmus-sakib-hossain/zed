...
TRASH
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/dx-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/dx-onboard-${case_name}.log"
  ...
  run_case_local_basic() {
    ...
    workspace_dir="$HOME/.dx/workspace"
    config_path="$HOME/.dx/dx.json"
    sessions_dir="$HOME/.dx/agents/main/sessions"
