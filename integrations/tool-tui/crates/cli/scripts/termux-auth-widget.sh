...
# Check auth status
termux-toast "Checking Dx auth..."

STATUS=$(ssh "$SERVER" '$HOME/dx/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/dx/scripts/claude-auth-status.sh json' 2>&1)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/dx/scripts/mobile-reauth.sh'"
                ;;
            *)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: Dx won't work until re-auth"
                ;;
        esac
        ;;
