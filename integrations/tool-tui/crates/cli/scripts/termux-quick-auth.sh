...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/dx/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Dx Re-Auth" -c "After getting key, run: ssh $SERVER '~/dx/scripts/mobile-reauth.sh'" --id dx-auth
        ;;
    *)
