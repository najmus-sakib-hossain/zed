#!/usr/bin/env bash
# Reset Dx like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${OPENCLAW_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="Dx.app/Contents/MacOS/Dx"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/Dx"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/Dx"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/Dx"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.dx.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/dx-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${OPENCLAW_RESTART_LOG:-/tmp/dx-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${OPENCLAW_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.dx/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log "  node dx.mjs daemon install --force --runtime node"
      log "  node dx.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.dx/disable-launchagent"
      log ""
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.dx.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing Dx instances"
kill_all_dx
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product Dx"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/Dx.app" ]]; then
    APP_BUNDLE="/Applications/Dx.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/Dx.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/Dx.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/Dx.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set OPENCLAW_APP_BUNDLE to your installed Dx.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node dx.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node dx.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".dx", "dx.json"), "utf8");
