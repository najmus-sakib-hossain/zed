...
export OPENCLAW_STATE_DIR="/tmp/dx-test"
export OPENCLAW_CONFIG_PATH="${OPENCLAW_STATE_DIR}/dx.json"
...
pnpm dx reset --scope config+creds+sessions --yes --non-interactive
...
pnpm dx uninstall --state --yes --non-interactive
