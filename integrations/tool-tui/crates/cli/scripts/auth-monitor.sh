...
#
# Suggested cron: */30 * * * * /home/admin/dx/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Dx notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., dx-alerts)
...
STATE_FILE="$HOME/.dx/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Dx if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use dx
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Dx to $NOTIFY_PHONE..."
            dx send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Dx is down. Run: ssh l36 '~/dx/scripts/mobile-reauth.sh'" "urgent"
    exit 1
