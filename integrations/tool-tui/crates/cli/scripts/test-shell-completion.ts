 ...
 * `dx update`. Use it to verify the completion installation flow
...
\`dx update\`, \`dx doctor\`, and \`dx onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Dx Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`dx completion --install\` later to enable.`));
    return;
  }
