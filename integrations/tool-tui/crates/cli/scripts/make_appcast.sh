...
ZIP=${1:?"Usage: $0 Dx-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/dx/dx/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Dx-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/dx/dx/releases/download/v${VERSION}/"}
