...
impl SyntaxCounter {
    ...
    pub(crate) fn parse_line_comment(&self, window: &[u8]) -> bool {
        if self.quote.is_some() || !self.stack.is_empty() {
            ...
            self.shared.line_comments.iter().find(|c| window.starts_with(c.as_bytes()))
        {
            trace!("Start {:?}", comment);
            true
        } else {
    ...
    pub(crate) fn try_perform_single_line_analysis(
        ...
        if !self.is_plain_mode() {
            ...
            if self.shared.is_literate
                || self.shared.line_comments.iter().any(|c| line.starts_with(c.as_bytes()))
            {
                stats.comments += 1;
    ...
    pub(crate) fn perform_multi_line_analysis(
        ...
        for i in start..end {
            ...
            let is_end_of_quote_or_multi_line =
                self.parse_end_of_quote(window).or_else(|| self.parse_end_of_multi_line(window));
            ...
            let is_quote_or_multi_line =
                self.parse_quote(window).or_else(|| self.parse_multi_line_comment(window));
    ...
    pub(crate) fn line_is_comment(
        ...
        let whole_line_is_comment = || {
            self.shared.line_comments.iter().any(|c| trimmed.starts_with(c.as_bytes()))
                || self.shared.any_multi_line_comments.iter().any(|(start, end)| {
        ...
        } else if self.shared.doc_quotes.iter().any(|(_, e)| line.contains_slice(e.as_bytes()))
            && started_in_comments
        {
            true
    ...
    pub(crate) fn parse_context(
        ...
        match regex_cache.family()? {
            ...
            RegexFamily::LinguaFranca(lf) => {
                ...
                Some(FileContext::new(LanguageContext::LinguaFranca, end_of_code, stats))
            }
            RegexFamily::HtmlLike(html) => {
                if let Some(mut captures) = html.start_script_in_range(start, end) {
                    ...
                    Some(FileContext::new(LanguageContext::Html { language }, end_of_code, stats))
    ...
    pub(crate) fn parse_quote(&mut self, window: &[u8]) -> Option<usize> {
        ...
        if let Some((start, end)) =
            self.shared.doc_quotes.iter().find(|(s, _)| window.starts_with(s.as_bytes()))
        {
    ...
    pub(crate) fn parse_multi_line_comment(&mut self, window: &[u8]) -> Option<usize> {
        ...
        let iter = self.shared.multi_line_comments.iter().chain(self.shared.nested_comments);
    ...
    pub(crate) fn parse_end_of_multi_line(&mut self, window: &[u8]) -> Option<usize> {
        if self.stack.last().is_some_and(|l| window.starts_with(l.as_bytes())) {
