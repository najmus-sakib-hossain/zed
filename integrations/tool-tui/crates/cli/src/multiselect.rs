    ...
    T: Clone + Eq,
{
    ...
    pub fn interact(&mut self) -> io::Result<Vec<T>> {
        if self.items.is_empty() {
            return Err(io::Error::new(io::ErrorKind::InvalidInput, "No items added to the list"));
        }
...
impl<T: Clone> PromptInteraction<Vec<T>> for MultiSelect<T> {
    ...
    fn render(&mut self, state: &State<Vec<T>>) -> String {
        ...
        let not_rendered_items = self.items.iter().filter(|i| i.borrow().selected).count()
            - self.filter.items().iter().filter(|i| i.borrow().selected).count();
...
mod test {
    ...
    fn empty_list() {
        ...
        assert_eq!("No items added to the list", result.unwrap_err().to_string());
    }
}
