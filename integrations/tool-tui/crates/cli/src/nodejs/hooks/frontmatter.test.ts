...
`;
    ...
    expect(parsed.dx.emoji).toBe("💾");
    expect(parsed.dx.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.dx.emoji).toBe("📝");
    expect(parsed.dx.events).toEqual(["command"]);
    expect(parsed.dx.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.dx.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"dx": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"dx": {"events": ["test"]}}');
  });
...
describe("resolveOpenClawMetadata", () => {
  it("extracts dx metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
  ...
  it("returns undefined when dx key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
      ...
      metadata: JSON.stringify({
        dx: {
          ...
          install: [
            ...
            { id: "npm", kind: "npm", package: "@dx/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@dx/hook");
  });
...
homepage: https://docs.dx.ai/hooks#session-memory
metadata:
  {
    "dx":
      {
...
`;
    ...
    const dx = resolveOpenClawMetadata(frontmatter);
    expect(dx).toBeDefined();
    expect(dx?.emoji).toBe("💾");
    expect(dx?.events).toEqual(["command:new"]);
    expect(dx?.requires?.config).toEqual(["workspace.dir"]);
    expect(dx?.install?.[0].kind).toBe("bundled");
  });
