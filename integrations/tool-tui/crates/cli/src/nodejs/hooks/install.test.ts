...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `dx-hook-install-${randomUUID()}`);
  ...
  it("installs hook packs from zip archives", async () => {
    ...
    zip.file(
      ...
      [
        ...
        'metadata: {"dx":{"events":["command:new"]}}',
        "---",
        "",
        "# Zip Hook",
      ].join("\n"),
