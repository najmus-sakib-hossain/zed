...
const saveSessionToMemory: HookHandler = async (event) => {
  ...
  try {
    ...
    const workspaceDir = cfg
      ...
      : path.join(os.homedir(), ".dx", "workspace");
    ...
    if (sessionFile) {
      ...
      if (sessionContent && cfg) {
        ...
        const dxRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../..");
        const slugGenPath = path.join(dxRoot, "llm-slug-generator.js");
