...
import { ensureDxCliOnPath } from "../infra/path-env.js";
...
export async function createAcpClient(opts: AcpClientOptions = {}): Promise<AcpClientHandle> {
  ...
  ensureDxCliOnPath({ cwd });
  const serverCommand = opts.serverCommand ?? "dx";
  ...
  await client.initialize({
    ...
    clientInfo: { name: "dx-acp-client", version: "1.0.0" },
  });
