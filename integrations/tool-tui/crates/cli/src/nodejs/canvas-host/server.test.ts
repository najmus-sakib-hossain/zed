...
describe("canvas host", () => {
  it("injects live reload script", () => {
    ...
    expect(out).toContain("dxCanvasA2UIAction");
    expect(out).toContain("dxSendUserAction");
  });

  it("creates a default index.html when missing", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "dx-canvas-"));
    ...
    try {
      ...
      expect(html).toContain("dxSendUserAction");
  ...
  it("serves the gateway-hosted A2UI scaffold", async () => {
    ...
    try {
      ...
      await fs.writeFile(bundlePath, "window.dxA2UI = {};", "utf8");
      createdBundle = true;
    }
    ...
    try {
      const res = await fetch(`http://127.0.0.1:${server.port}/__dx__/a2ui/`);
      const html = await res.text();
      expect(res.status).toBe(200);
      expect(html).toContain("dx-a2ui-host");
      expect(html).toContain("dxCanvasA2UIAction");

      const bundleRes = await fetch(
        `http://127.0.0.1:${server.port}/__dx__/a2ui/a2ui.bundle.js`,
      );
      const js = await bundleRes.text();
      expect(bundleRes.status).toBe(200);
      expect(js).toContain("dxA2UI");
    } finally {
      await server.close();
      if (createdBundle) {
