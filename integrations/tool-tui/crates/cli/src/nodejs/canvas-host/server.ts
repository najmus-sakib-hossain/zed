...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.dxCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.dxCanvasA2UIAction &&
        typeof window.dxCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.dxSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("dx:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Dx node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.dxSendUserAction === "function"
        ? window.dxSendUserAction
        : undefined;
...
function resolveDefaultCanvasRoot(): string {
  const candidates = [path.join(os.homedir(), ".dx", "canvas")];
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Dx Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
