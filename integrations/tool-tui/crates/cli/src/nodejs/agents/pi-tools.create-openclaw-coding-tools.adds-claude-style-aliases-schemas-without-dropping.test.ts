...
import { createDxTools } from "./dx-tools.js";
import { __testing, createDxCodingTools } from "./pi-tools.js";
...
describe("createDxCodingTools", () => {
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createDxCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "dx-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "dx-outside.txt");
