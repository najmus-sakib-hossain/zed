...
type WebSearchConfig = NonNullable<DxConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: DxConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  if (provider === "perplexity") {
    return {
      ...
      docs: "https://docs.dx.ai/tools/web",
    };
  }
  return {
    error: "missing_brave_api_key",
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("dx configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.dx.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://dx.ai",
      "X-Title": "Dx Web Search",
    },
    body: JSON.stringify({
      model: params.model,
      messages: [
