...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "dx.sandbox=1");
  args.push("--label", `dx.sessionKey=${params.scopeKey}`);
  args.push("--label", `dx.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `dx.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  ...
  return await readLabel("dx.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`dx sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`dx sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("dx sandbox recreate --all");
}
