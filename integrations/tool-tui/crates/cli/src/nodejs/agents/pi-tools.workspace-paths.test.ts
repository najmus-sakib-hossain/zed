...
import { createDxCodingTools } from "./pi-tools.js";
...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("dx-ws-", async (workspaceDir) => {
      await withTempDir("dx-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createDxCodingTools({ workspaceDir });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("dx-ws-", async (workspaceDir) => {
      await withTempDir("dx-override-", async (overrideDir) => {
        const tools = createDxCodingTools({ workspaceDir });
        const execTool = tools.find((tool) => tool.name === "exec");
        expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("dx-sandbox-", async (sandboxDir) => {
      await withTempDir("dx-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "dx-sbx-test",
          containerWorkdir: "/workspace",
          docker: {
            image: "dx-sandbox:bookworm-slim",
            containerPrefix: "dx-sbx-",
            workdir: "/workspace",
            readOnlyRoot: true,
            tmpfs: [],
            network: "none",
        ...
        const tools = createDxCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
