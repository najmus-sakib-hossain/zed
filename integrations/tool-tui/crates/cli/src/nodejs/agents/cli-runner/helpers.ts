...
/**
 * Cleanup suspended Dx CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: DxConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "dx-cli-images-"));
  const paths: string[] = [];
