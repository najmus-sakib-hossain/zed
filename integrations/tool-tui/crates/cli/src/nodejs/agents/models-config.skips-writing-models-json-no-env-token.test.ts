...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "dx-models-" });
}

const MODELS_CONFIG: DxConfig = {
  models: {
    providers: {
      "custom-proxy": {
...
describe("models-config", () => {
  ...
  it("skips writing models.json when no env token or profile exists", async () => {
    await withTempHome(async (home) => {
      ...
      try {
        vi.resetModules();
        const { ensureDxModelsJson } = await import("./models-config.js");

        const agentDir = path.join(home, "agent-empty");
        const result = await ensureDxModelsJson(
          {
            models: { providers: {} },
          },
          agentDir,
  ...
  it("writes models.json for configured providers", async () => {
    await withTempHome(async () => {
      vi.resetModules();
      const { ensureDxModelsJson } = await import("./models-config.js");
      const { resolveDxAgentDir } = await import("./agent-paths.js");

      await ensureDxModelsJson(MODELS_CONFIG);

      const modelPath = path.join(resolveDxAgentDir(), "models.json");
