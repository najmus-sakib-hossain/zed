...
const defaultTools = createDxCodingTools();

describe("createDxCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "dx-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    const tools = createDxCodingTools();
    ...
    try {
      ...
      const contents = "Hello from dx read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "dx-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "dx-workspace"),
    ...
    const tools = createDxCodingTools({ sandbox });
