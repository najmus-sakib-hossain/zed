...
import type { DxConfig } from "../config/types.js";
import type {
...
export function isCommandEnabled(cfg: DxConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: DxConfig): {
  provider: string;
  model: string;
} {
...
export function getCommandDetection(_cfg?: DxConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: DxConfig) {
