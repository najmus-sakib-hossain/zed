...
import { scheduleGatewaySigusr1Restart, triggerDxRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Dx in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerDxRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Dx via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
