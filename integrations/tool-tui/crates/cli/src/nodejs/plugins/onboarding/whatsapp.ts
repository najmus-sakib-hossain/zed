...
function setWhatsAppDmPolicy(cfg: DxConfig, dmPolicy: DmPolicy): DxConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: DxConfig, allowFrom?: string[]): DxConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: DxConfig, selfChatMode: boolean): DxConfig {
...
async function detectWhatsAppLinked(cfg: DxConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppAllowFrom(
  ...
  if (options?.forceAllowlist) {
    await prompter.note(
      "We need the sender/owner number so Dx can allowlist you.",
      "WhatsApp number",
    );
  ...
  const phoneMode = await prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for Dx" },
    ],
  });
