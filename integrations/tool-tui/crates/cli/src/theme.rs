...
const S_STEP_CANCEL: Emoji = Emoji("◆", "♦");
const S_STEP_ERROR: Emoji = Emoji("◆", "♦");
const S_STEP_SUBMIT: Emoji = Emoji("◆", "♦");
...
pub trait Theme {
    ...
    fn bar_color(&self, state: &ThemeState) -> Style {
        match state {
            ThemeState::Active => Style::new().bright().black(),
    ...
    fn radio_symbol(&self, state: &ThemeState, selected: bool) -> String {
        match state {
            ThemeState::Active if selected => style("›").green(),
    ...
    fn format_select_item(
        ...
        format!("│  {}\n", self.radio_item(state, selected, label, hint))
    }
