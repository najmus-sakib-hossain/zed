---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Dx deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Dx cron scheduling for periodic checks, or version status checks on a machine running Dx (laptop, workstation, Pi, VPS).
---
...
Assess and harden the host running Dx, then align it to a user-defined risk tolerance without breaking access. Use Dx security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Dx changes the host firewall, SSH, or OS updates; it does not.
...
5. Dx gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Dx can access sensitive data.
...
### 2) Run Dx security audits (read-only)

As part of the default read-only checks, run `dx security audit --deep`. Only offer alternatives if the user requests them:

1. `dx security audit` (faster, non-probing)
2. `dx security audit --json` (structured output)

Offer to apply Dx safe defaults (numbered):

1. `dx security audit --fix`

Be explicit that `--fix` only tightens Dx defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Dx version/update status (read-only)

As part of the default read-only checks, run `dx update status`.
...
- Credential hygiene notes (location of Dx creds, prefer disk encryption)
...
After Dx install or first hardening pass, run at least one baseline audit and version check:

- `dx security audit`
- `dx security audit --deep`
- `dx update status`

Ongoing monitoring is recommended. Use the Dx cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `dx cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `dx update status`
...
Before creating, `dx cron list` and match on exact `name`. If found, `dx cron edit <id> ...`.
If not found, `dx cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `dx update status` (preferred for source checkouts and channels)
2. `npm view dx version` (published npm version)

## Dx command accuracy

Use only supported commands and flags:

- `dx security audit [--deep] [--fix] [--json]`
- `dx status` / `dx status --deep`
- `dx health --json`
- `dx update status`
- `dx cron add|list|runs|run`

Do not invent CLI flags or imply Dx enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Dx compaction:
