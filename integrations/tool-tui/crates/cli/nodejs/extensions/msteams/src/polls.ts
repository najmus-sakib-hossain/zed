...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["dxPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["dx", "pollId"]) ??
    readNestedString(value, ["dx", "poll", "id"]) ??
    readNestedString(value, ["data", "dxPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "dx", "pollId"]);
  if (!pollId) {
    return null;
  }
