...
export async function handleNextcloudTalkInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as DxConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as DxConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as DxConfig);
