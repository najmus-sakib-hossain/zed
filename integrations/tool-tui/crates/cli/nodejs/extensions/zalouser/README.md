...
Dx extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
dx channels login --channel zalouser
...
```bash
dx message send --channel zalouser --target <threadId> --message "Hello from Dx!"
```
...
```bash
dx channels login --channel zalouser              # Login via QR
dx channels login --channel zalouser --account work
dx channels status --probe
dx channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
dx directory self --channel zalouser
dx directory peers list --channel zalouser --query "name"
dx directory groups list --channel zalouser --query "work"
dx directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
dx message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
dx message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `dx channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
dx channels login --channel zalouser --account work
dx message send --channel zalouser --account work --target <id> --message "Hello"
