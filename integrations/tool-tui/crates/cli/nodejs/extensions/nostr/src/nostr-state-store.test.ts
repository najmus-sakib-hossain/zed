import type { PluginRuntime } from "dx/plugin-sdk";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.DX_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "dx-nostr-"));
  process.env.DX_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const override = env.DX_STATE_DIR?.trim() || env.DX_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        return path.join(homedir(), ".dx");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      delete process.env.DX_STATE_DIR;
    } else {
      process.env.DX_STATE_DIR = previous;
    }
