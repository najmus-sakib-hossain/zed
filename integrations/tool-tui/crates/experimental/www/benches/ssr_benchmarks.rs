...
//!
//! Run with: `cargo bench --bench ssr_benchmarks`

use criterion::{black_box, criterion_group, criterion_main, BenchmarkId, Criterion, Throughput};
use dx_www_packet::{SlotDef, SlotType, Template};
use dx_www_server::ssr::{inflate_html, inflate_page, StateData};
...
fn generate_state(num_slots: usize) -> StateData {
    ...
    for i in 0..num_slots {
        state.set(i as u32, format!("Value for slot {} with some additional text to make it realistic", i));
    }
    state
}
...
fn bench_inflate_html(c: &mut Criterion) {
    ...
    for num_slots in slot_counts {
        ...
        group.bench_with_input(
            ...
            |b, (template, state)| {
                b.iter(|| {
                    black_box(inflate_html(black_box(template), black_box(state)))
                });
            },
        );
    }
...
fn bench_inflate_page(c: &mut Criterion) {
    ...
    for num_slots in slot_counts {
        ...
        let meta_tags = vec![
            ("description".to_string(), "A benchmark test page for SSR performance".to_string()),
...
fn bench_pages_per_second(c: &mut Criterion) {
    ...
    dashboard_state.set(1, "<a href='/'>Home</a> | <a href='/users'>Users</a> | <a href='/settings'>Settings</a>".to_string());
    ...
    dashboard_state.set(7, "<table><tr><th>ID</th><th>Name</th><th>Status</th></tr></table>".to_string());
    ...
    blog_state.set(6, "<button>Share on Twitter</button><button>Share on LinkedIn</button>".to_string());
